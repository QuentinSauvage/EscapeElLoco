...
GameLogic::GameLogic(GameAudio &audio) : gameAudio(audio), level(1), timer(0), pause(false) {
	extractMap();
	...
	if(!pause) {
		...
		if(sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {
			if(players[0].state != 2) {
				...
				players[0].state = 2;
				if(gameAudio.playing) gameAudio.jump_sound.play();
			}
...
void GameLogic::jump(int p_index, vector<vector<sf::Sprite>> &gmap, float deltaTime) {
	if(players[p_index].state == 2) {
        players[p_index].vy -= ((GRAVITY * GRAVITY) * deltaTime);
		players[p_index].dy = players[p_index].y;
...
void GameLogic::handleCollisions(int p_index, vector<vector<sf::Sprite>> &gmap, float deltaTime) {
	...
	float xp, xn, yp, yn;
	int indY = players[p_index].y/TILE_DIM;
    int indX = players[p_index].x/TILE_DIM;
	sf::FloatRect rect;
	sf::FloatRect r = players[p_index].sprite.getGlobalBounds();

	if(map.map[indY+1][indX] == EMPTY && players[p_index].state != 2) {
	...
	for(; yn<yp; yn++)
		if(map.map[yn][indX] != EMPTY && (players[p_index].sprite.getGlobalBounds().intersects(gmap[yn][indX].getGlobalBounds(),rect))) {
			if(rect.top > r.top) {
	...
	for(; xn<xp; xn++)
		if(map.map[indY][xn] != EMPTY && (players[p_index].sprite.getGlobalBounds().intersects(gmap[indY][xn].getGlobalBounds(),rect))) {
...
void GameLogic::setPlayerSprite(int p_index, sf::Sprite &s) {
	players[p_index].sprite = s;
}
