...
Player::Player() : Object(), life(DEFAULT_MAX_LIFE), maxLife(DEFAULT_MAX_LIFE), speed(DEFAULT_SPEED), spriteRect(IDLE_1_OFFSET, OFFSET_Y, PLAYER_DIM, PLAYER_DIM) {
	vx=0, vy=0,dx=0,dy=0;
...
void Player::animate(float timer) {
	...
	if(state == 2) {
		if(vy < -1)
			spriteRect.left = JUMP_2_OFFSET;
		else if(vy > 1)
			spriteRect.left = JUMP_1_OFFSET;
		else
			spriteRect.left = JUMP_3_OFFSET;
		if(vx < 0)
			sprite.setScale(-4.f,4.f);
		else if(vx > 0)
			sprite.setScale(4.f,4.f);
